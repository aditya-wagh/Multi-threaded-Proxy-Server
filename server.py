...
class Server:
    ...
    def __init__(self):
        print("#####System Port : ",server_port)
        try:
            # Internet,TCP
            self.server_socket = socket(AF_INET, SOCK_STREAM)  
            #Allow Socket to rebind to address and port already in use
            self.server_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1) 
        except error as e:
            print ('Unable to create/re-use the socket. Error: ',e)
    ...
    def proxy_thread(self, client_connection_socket, client_address):
        ...
        if client_request:
            ...
            print("12",client_request)
            
            print(type(client_request))
            resp_part = str(client_request).split(' ')[0]
            print(resp_part)
            if resp_part == "GET":
                print("3")
            ...
            print("Bad Request")
            client_connection_socket.send(bytes("",'utf-8'))
            client_connection_socket.close()
    ...
    def find_file(self, url_file_name, client_connection_socket, port_number, client_address, start_time, url_connect, url_slash):
        try:
            ...
            ans=bytes(response_message,'utf-8')
            client_connection_socket.send(ans)
            end_time = time.time()
            ...
            try:
                ...
                print ('Unable to create the socket. Error:',e)
            ...
            try:
                ...
                if url_slash:
                    ...
                    web_request = bytes(web_request, 'utf-8')
                ...
                while True:
                    ...
                    if len(web_server_response) > 0:
                        web_server_response=web_server_response.decode()
                ...
                web_server_response_append=bytes(web_server_response_append,'utf-8')
                response_to_file = web_server_response_append
                # appending web server details to the response sent to client
                web_server_response_append=web_server_response_append.decode()
